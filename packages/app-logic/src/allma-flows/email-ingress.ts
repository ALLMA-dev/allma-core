...
import { postSimpleJson } from '../allma-core/utils/api-executor.js'; // FIX: Import from local path
...
export const handler = async (event: { Records: SesEventRecord[] }): Promise<void> => {
    for (const record of event.Records) {
        ...
        const { bucketName, objectKey } = record.ses.receipt.action;
        log_info('Processing inbound email from SES', { bucketName, objectKey }, correlationId);

        try {
            // 1. Download email from S3
            const s3Response = await s3Client.send(new GetObjectCommand({ Bucket: bucketName, Key: objectKey }));
            if (!s3Response.Body) {
            ...
            // We re-throw the error to ensure the SES receipt rule's invocation is marked as failed.
            // This can be useful for monitoring and alerts.
            throw error;
        }
    }
};