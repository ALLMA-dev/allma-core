    ...
    for (const record of event.Records) {
        ...
        try {
            ...
            const triggeringEmail: TriggeringEmailContext = {
                ...
                to: recipient,
                cc: parsedEmail.cc?.map(addr => addr.address).filter((addr): addr is string => !!addr),
                bcc: parsedEmail.bcc?.map(addr => addr.address).filter((addr): addr is string => !!addr),
                replyTo: parsedEmail.replyTo?.map(addr => addr.address).filter((addr): addr is string => !!addr),
