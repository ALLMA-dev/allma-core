...
async function sendCfnResponse(
  ...
  try {
    await fetch(event.ResponseURL, responseOptions);
    console.log('Successfully sent CloudFormation response.');
  } catch (error) {
    console.error('Failed to send CloudFormation response:', error);
    // This is a critical failure, but we can't do much more than log it.
  }
}
 ...
 */
export const handler = async (event: CfnCustomResourceEvent): Promise<void> => {
