...
export function FlowVersionList({ flowId }: FlowVersionListProps) {
  ...
  const handleTestClick = (version: FlowVersionSummary) => {
    setVersionToModify(version);
    openTestModal();
  };
    ...
    <Table.Tr key={version.version}>
      ...
      <Table.Td>
        <Group gap="xs">
          ...
          <Tooltip label="Run Test Execution"><ActionIcon variant="subtle" color="gray" onClick={() => handleTestClick(version)}><IconPlayerPlay size="1rem" /></ActionIcon></Tooltip>
        </Group>
      </Table.Td>
    </Table.Tr>
  ));
  ...
  return (
    <>
      ...
      <Modal opened={publishModal} onClose={closePublishModal} title="Confirm Publish" centered><Text>Are you sure you want to publish <Text span fw={700}>version {versionToModify?.version}</Text> of the flow <Text span fw={700}>&ldquo;{flowConfig?.name}&rdquo;</Text>?</Text><Text c="dimmed" size="sm" mt="sm">This will become the active version for all new executions. If another version is currently published, it will be unpublished.</Text><Group justify="flex-end" mt="xl"><Button variant="default" onClick={closePublishModal}>Cancel</Button><Button color="green" onClick={confirmPublish} loading={publishMutation.isPending}>Publish Version</Button></Group></Modal>
      <Modal opened={unpublishModal} onClose={closeUnpublishModal} title="Confirm Unpublish" centered><Text>Are you sure you want to unpublish <Text span fw={700}>version {versionToModify?.version}</Text>?</Text><Text c="dimmed" size="sm" mt="sm">This will remove the &ldquo;Published&rdquo; status, and there will be no active version for this flow until a new one is published.</Text><Group justify="flex-end" mt="xl"><Button variant="default" onClick={closeUnpublishModal}>Cancel</Button><Button color="orange" onClick={confirmUnpublish} loading={unpublishMutation.isPending}>Unpublish</Button></Group></Modal>
      <TestExecutionModal
