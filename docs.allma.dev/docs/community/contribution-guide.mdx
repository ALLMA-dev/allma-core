...
2.  Run the following command:
    ```bash
    npm run dev --workspace=@allma/admin-shell
    ```
...
-   **Automated Versioning:** We rely on **Changesets** to automate package versioning and changelog generation.
...
1.  **Find an Issue:** Browse our [GitHub Issues](https://github.com/ALLMA-dev/allma-core/issues). Look for labels like `good first issue` or `help wanted`.
...
1.  **Fork:** Go to the [Allma repository](https://github.com/ALLMA-dev/allma-core) on GitHub and click the "Fork" button.
2.  **Clone:** Clone your forked repository to your local machine.
    ```bash
    git clone https://github.com/YOUR_USERNAME/allma-core.git
    cd allma-core
    ```
...
Commit your work as you go with clear, descriptive messages. While we no longer require Conventional Commits for versioning, a good commit message is still vital for code reviews.
...
git commit -m "feat: add string case converter module"
```

### Step 4: Add a Changeset (Required for Features & Fixes)

This is the most important step for our automated release process. If your changes should be included in the next release (i.e., you've added a feature or fixed a bug), you **must** create a "changeset".

1.  **Run the command:** From the root of the project, run:
    ```bash
    npm run changeset
    ```

2.  **Follow the Prompts:** The tool will guide you through a series of questions:
    -   **Select packages:** Use the arrow keys and spacebar to select all the packages your PR has modified.
    -   **Select version bump:** Choose `major`, `minor`, or `patch` for each package. A new feature is typically a `minor` bump, and a bug fix is a `patch`.
    -   **Write a summary:** Write a clear, concise summary of your changes. This summary will be used to generate the official `CHANGELOG.md` for the packages.

3.  **Commit the Changeset File:** The command will generate a new markdown file in the `.changeset/` directory. **You must commit this file to your branch.** This file is how our release bot knows what to do.

### Step 5: Keep Your Branch in Sync (Important!)
...
1.  **Add the "Upstream" Remote (One-time setup):**
    ```bash
    git remote add upstream https://github.com/ALLMA-dev/allma-core.git
    ```
...
Push your committed changes—including the new changeset file—to your forked repository on GitHub.

```bash
# The -f flag may be needed if you rebased an already-pushed branch
...
2.  **Set the Base:** Ensure the PR is targeting the `ALLMA-dev/allma-core` `main` branch.
...
-   **Maintainer Review:** A maintainer will review your code. They may request changes—this is a normal and healthy part of collaboration!
...
Once your PR is approved and all checks pass, a maintainer will merge it. Congratulations, and thank you for your contribution! 🎉