...
1.  **Create the file:**
    `packages/core-types/src/steps/data-transformation/string-case-converter.ts`

2.  **Add the schema:**
    This schema defines the "API" for our module. Anyone using it must provide these fields.

    ```typescript title="packages/core-types/src/steps/data-transformation/string-case-converter.ts"
    import { z } from 'zod';
...
1.  **Create the file:**
    `packages/app-logic/src/allma-core/data-transformers/string-case-converter.ts`

2.  **Add the handler logic:**

    ```typescript title="packages/app-logic/src/allma-core/data-transformers/string-case-converter.ts"
...
1.  **Create the test file:**
    `packages/app-logic/src/allma-core/data-transformers/__tests__/string-case-converter.test.ts`

2.  **Add the tests:**

    ```typescript title="packages/app-logic/src/allma-core/data-transformers/__tests__/string-case-converter.test.ts"
...
1.  **Add a System Identifier:** First, give your module a unique identifier.
    Modify `packages/core-types/src/steps/system-module-identifiers.ts`.

    ```typescript title="packages/core-types/src/steps/system-module-identifiers.ts"
...
2.  **Register the Handler:** Now, connect that identifier to your handler function.
    Modify `packages/app-logic/src/allma-core/step-handlers/data-transformation-handler.ts`.

    ```typescript title="packages/app-logic/src/allma-core/step-handlers/data-transformation-handler.ts"
...
Now that the feature is working, we need to tell our release system how to version the packages you changed.
...
2.  **Select the packages** you modified. For this tutorial, you touched `@allma/core-types` (for the schema) and `allma-app-logic` (for the handler). Use the arrow keys and spacebar to select them, then press Enter.

3.  **Choose the version bump.** Since we added a new feature, this is a `minor` change for both packages. Select `minor` for each.

4.  **Write the summary.** This text will appear in the changelog.
    ```
    feat(logic): add string-case-converter module

    This adds a new DATA_TRANSFORMATION module that can
    convert a given string to either UPPERCASE or lowercase.
    ```
    Press Enter to confirm.

5.  **Commit the new file.** A new file like `.changeset/pretty-dolphins-laugh.md` has been created. Commit it to your branch.
    ```bash
    git add .
    git commit -m "chore: add changeset for case-converter module"
    ```

You have successfully added a new feature to Allma. The final step is to follow our **[Contribution Workflow Guide](./contribution-guide)** to push your branch and open a Pull Request.
